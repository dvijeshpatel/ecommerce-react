...
function CheckOutTableTitle(props){
    return <thead><tr className="cart-table__row cart-table__row-head">
    ...
    render() {
        return <tr className="cart-table__row">
            ...
            <td className="cart-table__col">
                <button className="btn-plus" data-id={this.props.itemInCart.item.itemId} data-change="1" onClick={this.props.onClickHandlerPlusMinus}>+</button>
                <input type="text" data-id={this.props.itemInCart.item.itemId} className="cart-table__quantity" value={this.props.itemInCart.quantityInCart}
                       data-old-quantity={this.props.itemInCart.quantityInCart}
                       onChange={this.props.onQuantityChangeHandler}/>
                <button className="btn-minus" data-id={this.props.itemInCart.item.itemId} data-change="-1" onClick={this.props.onClickHandlerPlusMinus}>-</button>
            </td>
            <td className="cart-table__col">
                <button className="remove-button" data-button-id={this.props.itemInCart.item.itemId} onClick={this.props.removeItem}>
                    Remove
...
class CheckOutTable extends React.Component{
    ...
    onQuantityChangeHandler(event){
        ...
        let idx = this.props.itemsInCart.findIndex((element)=>{return element.item.itemId==itemId});
    ...
    removeItem(event){
        let itemId = event.target.dataset.buttonId;
        //console.log(itemId);
        const duplicate = this.props.itemsInCart.filter(element=>{return element.item.itemId!=itemId});
        this.props.updateItemsInCart(duplicate);
    ...
    render(){
        ...
        const cartTableItemsDisplay = this.props.itemsInCart.map(element=><CheckOutItem key={element.item.itemId} itemInCart={element}
                                                                                        onClickHandlerPlusMinus={this.onClickHandlerPlusMinus}
                                                                                        onQuantityChangeHandler={this.onQuantityChangeHandler}
                                                                                        removeItem={this.removeItem}     />)
        const returnElement = <div className="checkOutTableWrapper"><table className="cart-table">
            <CheckOutTableTitle/>
            <tbody>
            {cartTableItemsDisplay}

            <CheckOutTableFooter totalItems={this.countTotalItemsInCart()} totalPrice={this.countTotalPriceOfCart()}/>
            </tbody>
        </table>
        <Form/>
         </div>;
